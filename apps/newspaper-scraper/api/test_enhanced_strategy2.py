...
def test_enhanced_strategy2_only():
    ...
    try:
        ...
        for i, url in enumerate(results, 1):
            ...
            url_parts = url.split('/')
            if len(url_parts) > 1:
                title_slug = url_parts[-1].replace('-',
                                                   ' ').replace('.html', '')
                title = title_slug[:60] + \
                    "..." if len(title_slug) > 60 else title_slug
            else:
        ...
        print("-" * 50)
        print(
            f"📍 Shellharbour matches: {shellharbour_count}/{len(results)} ({shellharbour_count/len(results)*100:.1f}%)")
        print(
            f"🏛️  Council matches: {council_count}/{len(results)} ({council_count/len(results)*100:.1f}%)")
        print(
            f"🎯 Perfect matches: {len([url for url in results if 'shellharbour' in url.lower() and 'council' in url.lower()])}")
        ...
        relevance_pct = (shellharbour_count + council_count) / \
