        ...
        for page in range(1, 11):  # Scan up to 10 pages deep
            ...
            try:
                ...
                if response.status_code != 200:
                    if page == 1:
                        print(
                            f'   ❌ Category not accessible: {response.status_code}')
                    break
                ...
                if page_found:
                    print(
                        f'   📄 Page {page}: {len(article_links)} articles - 🎯 FOUND {len(page_found)} TARGETS!')
                    ...
                    if story_ids:
                        print(
                            f'   📄 Page {page}: {len(article_links)} articles (IDs: {min(story_ids)}-{max(story_ids)})')
                    else:
                        print(
                            f'   📄 Page {page}: {len(article_links)} articles')
    ...
    if all_found:
        ...
        for target_id, link, category, page in all_found:
            category_name = category.split(
                '/')[-2] if category.endswith('/') else category.split('/')[-1]
