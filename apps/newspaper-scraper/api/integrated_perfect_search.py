...
def perfect_match_search_illawarra_mercury(query, max_results=20):
    ...
    try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if relevance_score > 0:
                        ...
                        print(
                            f"   ✅ Found target: {story_id} - {title[:50]}...")
        ...
        if story_id_results:
            ...
            print(
                f"   🎯 Direct validation found {len(story_id_results)} perfect matches")
        ...
        if rss_results:
            ...
            print(
                f"   📡 RSS analysis found {len(rss_results)} relevant articles")
        ...
        if len(results['articles']) < max_results:
            ...
            if category_results:
                ...
                results['strategies_used'].append(
                    'comprehensive_category_discovery')
                results['performance']['categories_scanned'] = len(
                    get_working_categories())
                print(
                    f"   📊 Category discovery found {len(category_results)} relevant articles")
        ...
        results['performance']['success_rate'] = len(
            results['articles']) / max_results if max_results > 0 else 0
        results['performance']['perfect_match_rate'] = len(
            results['perfect_matches']) / len(results['articles']) if results['articles'] else 0
        ...
        print(
            f"   🎯 Target articles found: {len(results['target_articles_found'])}")
        print(f"   ⏱️ Duration: {results['performance']['duration']:.2f}s")
        print(
            f"   📈 Perfect match rate: {results['performance']['perfect_match_rate']:.1%}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if story_urls:
                ...
                # Every 3rd ID in recent range
                scan_range = range(max_id - 100, max_id, 3)
                ...
                for story_id in scan_range:
                    ...
                    try:
                        story_url = f"https://www.illawarramercury.com.au/story/{story_id}/"
                        response = requests.get(
                            story_url, headers=headers, timeout=5)
...
def comprehensive_category_discovery(query, max_results=10, headers=None):
    ...
    for category in working_categories[:10]:
        if len(articles) >= max_results:
            break
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Sample first 5 per category
                for article_url in list(article_links)[:5]:
                    if len(articles) >= max_results:
                        break
                    ...
                    try:
                        ...
                        if url_relevance > 0:
                            ...
                            article_response = requests.get(
                                article_url, headers=headers, timeout=5)
                            if article_response.status_code == 200:
                                article_soup = BeautifulSoup(
                                    article_response.content, 'html.parser')
                                title_elem = article_soup.find(
                                    'h1') or article_soup.find('title')
                                title = title_elem.get_text(
                                    strip=True) if title_elem else "Article"
...
def print_search_results(results):
    ...
    if results['perfect_matches']:
        ...
        for i, article in enumerate(results['perfect_matches'][:5], 1):
            ...
            print(
                f"      Score: {article['relevance_score']} | Source: {article['source']}")
