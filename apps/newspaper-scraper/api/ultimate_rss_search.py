...
def ultimate_rss_enhanced_search(query="shellharbour council", max_results=20):
        ...
        try:
            ...
            for page in range(1, 3):
                ...
                if response.status_code == 200:
                    ...
                    for item in items:
                        article = extract_rss_article(
                            item, query_terms, feed_url)
    ...
    print(
        f"   📡 RSS scan: {total_rss_scanned} articles scanned, {len(rss_results)} relevant found")
    ...
    print(
        f"   🎯 Direct validation: {len(direct_results)} perfect matches found")

    # Phase 3: Enhanced Story ID Range Scanning
    print("\n🔍 PHASE 3: Enhanced Story ID Range Scanning")
    range_results = enhanced_story_range_scan(
        query_terms, headers, max_articles=10)
    results['sources']['story_id_scan'] = len(range_results)
    print(
        f"   📈 Range scan: {len(range_results)} additional relevant articles found")
...
def print_ultimate_results(results):
    ...
    print(
        f"   • RSS articles scanned: {results['performance']['rss_articles_scanned']}")
    print(f"   • Duration: {results['performance']['duration']:.2f}s")
    print(
        f"   • Sources: RSS({results['sources']['rss_feeds']}) + Direct({results['sources']['direct_validation']}) + Range({results['sources']['story_id_scan']})")
    ...
    if results['articles']:
        ...
        for i, article in enumerate(results['articles'][:10], 1):
            ...
            print(
                f"   {marker} {i}. Score {article['relevance_score']}: {article['title'][:60]}...")
            print(
                f"      Source: {article['source']} | ID: {article.get('story_id', 'N/A')}")
    ...
    if results['rss_articles']:
        print(f"\n📡 RSS FEED HIGHLIGHTS:")
        rss_articles = sorted(
            results['rss_articles'], key=lambda x: x['relevance_score'], reverse=True)
        for article in rss_articles[:3]:
            print(
                f"   • {article['title'][:50]}... (Score: {article['relevance_score']})")
