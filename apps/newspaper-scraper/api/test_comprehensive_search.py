    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                avg_score = sum(article.get('score', 0) for article in data['results']) / len(
                    data['results']) if data['results'] else 0
                high_relevance = sum(
                    1 for article in data['results'] if article.get('score', 0) >= 8.0)
                perfect_matches = sum(
                    1 for article in data['results'] if article.get('score', 0) >= 9.0)

                print(f"📊 Quality Metrics:")
                print(f"   • Average relevance score: {avg_score:.1f}/10")
                print(f"   • High relevance (8.0+): {high_relevance} articles")
                print(
                    f"   • Perfect matches (9.0+): {perfect_matches} articles")
