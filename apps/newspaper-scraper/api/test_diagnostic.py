...
def check_diagnostic():
    ...
    try:
        response = requests.get(
            'https://news.prestigecorp.au/api/debug', timeout=30)
        ...
        if response.status_code == 200:
            ...
            # Key diagnostic info
            print(
                f'Search.py exists: {data.get("search_py_exists", "unknown")}')
            print(
                f'Contains Google CSE: {data.get("search_py_contains_google_cse", "unknown")}')
            print(f'File size: {data.get("search_py_size", "unknown")} chars')
            print(
                f'Requests available: {data.get("requests_available", "unknown")}')
