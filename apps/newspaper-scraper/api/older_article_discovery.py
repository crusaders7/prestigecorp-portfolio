...
class OlderArticleDiscovery:
        ...
        for period in [7, 14, 30, 60, 90, 180, 365]:
            if period <= days_back:
                ...
                ranges[f'{period}_days_ago'] = {
                    # Buffer
                    'estimated_start': max(estimated_id - 1000, 1000000),
    ...
    def systematic_id_backtrack(self, query, days_back=30, max_articles=50):
        ...
        for period, range_info in ranges.items():
            ...
            sample_step = max(50, (end_id - start_id) //
                              100)  # Sample 100 IDs max
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for future in as_completed(future_to_id):
                    ...
                    if article and article['relevance_score'] > 0:
                        ...
                        print(
                            f"   ✅ Found: {article['title'][:50]}... (ID: {article['story_id']})")
    ...
    def search_engine_discovery(self, query, days_back=30):
        ...
        simulated_results = [
            {'title': 'Historical Shellharbour Council Meeting Minutes',
                'estimated_date': '2024-07-15'},
            {'title': 'Shellharbour Development Approvals Process',
                'estimated_date': '2024-07-08'},
            {'title': 'Council Budget Discussions for Shellharbour',
                'estimated_date': '2024-06-25'}
        ]

        print(
            f"   📊 Search engine would find ~{len(simulated_results)} additional articles")
    ...
    def internet_archive_discovery(self, query, days_back=90):
        ...
        for target_date in target_dates[:5]:  # Limit to 5 snapshots
            ...
            try:
                ...
                print(
                    f"      📞 Would query: {wayback_api}?url={target_url}&timestamp={timestamp}")
        ...
        print(
            f"   📊 Archive discovery would find ~{sum(r.get('estimated_articles', 0) for r in results)} historical articles")
        return results
    ...
    def rss_deep_pagination(self, query, max_pages=50):
                    ...
                    if response.status_code == 200:
                        ...
                        if items:
                            ...
                            for item in items[:5]:  # Check first 5 items
                                article = self.extract_rss_article(
                                    item, query_terms)
                                if article and article['relevance_score'] > 0:
                            ...
                            print(
                                f"      ✅ Page {page}: {len(items)} articles")
                        else:
        ...
        print(
            f"   📊 RSS deep pagination found {len(results)} relevant articles")
        return results
    ...
    def comprehensive_older_article_search(self, query, days_back=30, max_results=20):
        ...
        # Strategy 1: Systematic ID Backtracking
        backtrack_results = self.systematic_id_backtrack(
            query, days_back, max_results // 2)
...
def main():
    ...
    try:
        results = discovery.comprehensive_older_article_search(
            query, days_back, max_results=15)
