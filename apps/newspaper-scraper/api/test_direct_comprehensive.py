...
def test_comprehensive_search_direct():
    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        try:
            ...
            if results:
                avg_score = sum(article.get('score', 0)
                                for article in results) / len(results)
                high_relevance = sum(
                    1 for article in results if article.get('score', 0) >= 8.0)
                perfect_matches = sum(
                    1 for article in results if article.get('score', 0) >= 9.0)
