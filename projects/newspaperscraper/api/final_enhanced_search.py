...
class EnhancedSearchSystem:
    ...
    def scrape_category_with_pagination(self, category_url, max_pages=10):
        ...
        for page in range(1, max_pages + 1):
            try:
                        ...
                        if response.status_code == 200:
                            ...
                            for script in soup.find_all('script', type='application/json'):
                                try:
                                    ...
                                    story_urls = re.findall(r'https://www\.illawarramercury\.com\.au/story/\d+/[^"]+', json_str)
                ...
                if page_targets:
                    print(f"   📄 Page {page}: {len(page_articles)} articles - 🎯 FOUND {len(page_targets)} TARGETS!")
                    for target_id, url in page_targets:
                        print(f"      • {target_id}: {self.target_articles[target_id]}")
                else:
                    ...
                    if sample_ids:
                        print(f"   📄 Page {page}: {len(page_articles)} articles (IDs: {min(sample_ids)}-{max(sample_ids)})")
                    else:
    ...
    def test_direct_story_id_scanning(self):
        ...
        try:
            response = requests.get('https://www.illawarramercury.com.au/rss.xml', headers=self.headers)
            if response.status_code == 200:
                ...
                if rss_ids:
                    ...
                    target_ids = [int(id_str) for id_str in self.target_articles.keys() if id_str.isdigit()]
                            ...
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    if test_id == target_id:
                                        print(f"   🎯 {test_id}: ✅ TARGET FOUND - {title[:80]}...")
                                        found_articles.append((str(test_id), test_url, title))
                                    else:
                                        print(f"   📄 {test_id}: ✅ {title[:60]}...")
                                else:
                                    marker = "🎯" if test_id == target_id else "📄"
                                    print(f"   {marker} {test_id}: ❌ Status {response.status_code}")
                                
                                time.sleep(0.3)
                                
                            except Exception as e:
                                marker = "🎯" if test_id == target_id else "📄"
                                print(f"   {marker} {test_id}: ❌ Error: {str(e)}")
    ...
    def run_comprehensive_search(self, query="shellharbour council"):
        ...
        for category in self.tier1_categories:
            articles, targets = self.scrape_category_with_pagination(category, max_pages=15)
        ...
        for category in self.extended_categories:
            articles, targets = self.scrape_category_with_pagination(category, max_pages=5)
            all_found_articles.extend(articles)
            all_found_targets.extend(targets)
    ...
    def analyze_relevance(self, articles, query):
        ...
        print(f"🔍 Analyzing {len(set(articles))} unique articles for relevance...")
...
def main():
    ...
    print(f"🎯 Target articles found: {len(results['found_targets'])}/{len(search_system.target_articles)}")
    print(f"📈 Target completion rate: {results['target_completion']:.1f}%")
    print(f"📝 Relevant articles identified: {len(results['relevant_articles'])}")
    
    if results['found_targets']:
        print("\n🎯 FOUND TARGET ARTICLES:")
        for target_id, url, *extra in results['found_targets']:
            title = search_system.target_articles.get(target_id, "Unknown title")
