...
def create_test_handler_original():
    ...
    class TestHandler:
        ...
        def search_illawarra_mercury(self, query, max_results):
            ...
            search_method = types.MethodType(original_handler.search_illawarra_mercury, dummy)
...
def create_test_handler_optimized():
    ...
    class TestHandler:
        ...
        def search_illawarra_mercury_optimized(self, query, max_results):
            import types
            dummy = type('DummyHandler', (), {'get_random_user_agent': self.get_random_user_agent})()
            search_method = types.MethodType(optimized_handler.search_illawarra_mercury_optimized, dummy)
            return search_method(query, max_results)
...
def main():
    ...
    if original_results.get('success'):
        ...
        print(f"🔵 Original Search: ❌ Failed - {original_results.get('error', 'Unknown error')}")
    ...
    if optimized_results.get('success'):
        ...
        print(f"🟢 Optimized Search: ❌ Failed - {optimized_results.get('error', 'Unknown error')}")
    
    # Performance improvement analysis
    if original_results.get('success') and optimized_results.get('success'):
        time_improvement = original_results['duration'] - optimized_results['duration']
        time_improvement_pct = (time_improvement / original_results['duration']) * 100
        
        result_improvement = optimized_results['count'] - original_results['count']
        
        print(f"\n🎯 OPTIMIZATION RESULTS:")
        print(f"   ⚡ Time improvement: {time_improvement:.2f}s ({time_improvement_pct:+.1f}%)")
