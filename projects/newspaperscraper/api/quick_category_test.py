...
def quick_category_test():
    ...
    for category in test_categories:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if article_count > 0:
                    working_categories.append((category, test_url, article_count))
                    print(f"✅ {category or 'homepage':<20} - {article_count:>3} articles")
                else:
                    print(f"⚠️  {category or 'homepage':<20} - No articles")
            else:
                print(f"❌ {category or 'homepage':<20} - Status {response.status_code}")
    ...
    for category, url, count in working_categories:
        if count >= 5:  # Only categories with decent content
            print(f'    "{url}",  # {category or "homepage"} - {count} articles')
