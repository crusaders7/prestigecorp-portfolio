...
class GoogleAPIKeyTester:
    ...
    def _test_keys_sequential(self, key_list: List[Dict]) -> List[Dict]:
        ...
        for i, key_info in enumerate(key_list, 1):
            ...
            print(
                f"🔍 Testing {i}/{len(key_list)}: {api_key[:25]}... (from {source})")
    ...
    def _test_single_api_key(self, api_key: str) -> Dict:
        ...
        try:
            ...
            response = requests.get(
                self.api_endpoint, params=params, timeout=15)
    ...
    def _generate_final_report(self, valid_keys: List[Dict], total_tested: int):
        ...
        if valid_keys:
            ...
            for i, key_info in enumerate(valid_keys, 1):
                ...
                print(
                    f"  📊 Total Results Available: {result.get('total_results', 'Unknown')}")
                print(
                    f"  ⚡ Response Time: {result.get('response_time', 'Unknown')}s")
                print(
                    f"  🕒 Search Time: {result.get('search_time', 'Unknown')}s")
