...
class IntelligentOlderArticleFinder:
    ...
    def google_site_search_strategy(self, query, days_back=30):
        ...
        search_variations = [
            ...
            f'site:illawarramercury.com.au {query} -"breaking news"'  # Exclude breaking news
        ]
        ...
        simulated_google_results = self.simulate_google_search_results(query, days_back)
        ...
        print(f"   📊 Google search strategy found {len(results)} verified articles")
        return results
    ...
    def internet_archive_strategy(self, query, days_back=90):
        ...
        print(f"   📅 Checking {len(target_dates)} potential snapshots across {len(target_urls)} URLs")
        ...
        for url in target_urls:
            ...
            for target_date in target_dates[:5]:  # Check 5 most recent
                ...
                try:
                    ...
                    snapshot_available = random.choice([True, False])  # Simulate availability
                    ...
                    if snapshot_available:
                        ...
                        simulated_articles = self.simulate_archive_articles(query, target_date)
    ...
    def simulate_archive_articles(self, query, snapshot_date):
        ...
        for i in range(2):  # 2 articles per snapshot
            ...
            estimated_id = 9053686 - int(1671.0 * days_ago) + random.randint(-500, 500)
    ...
    def smart_id_pattern_analysis(self, query, days_back=60):
        ...
        predicted_ranges = self.predict_historical_ranges(current_patterns, days_back)
        
        print(f"   🎯 Predicted ranges for last {days_back} days:")
        for period, range_info in predicted_ranges.items():
            print(f"      {period}: {range_info['start']} - {range_info['end']} (confidence: {range_info['confidence']:.1%})")
        
        # Smart sampling: focus on high-confidence ranges
        high_confidence_ranges = {k: v for k, v in predicted_ranges.items() if v['confidence'] > 0.7}
        ...
        for period, range_info in high_confidence_ranges.items():
            ...
            for story_id in strategic_ids[:20]:  # Limit to 20 per range
                ...
                if article:
                    ...
                    results.append(article)
                    found_in_range += 1
                    print(f"      ✅ Found article {story_id}: {article['title'][:40]}...")
    ...
    def analyze_current_id_patterns(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                patterns['publication_rate'] = patterns['count'] / 1  # Articles per day (RSS typically 1 day)
    ...
    def predict_historical_ranges(self, patterns, days_back):
        ...
        daily_rate = patterns.get('publication_rate', 1671.0)  # Fallback to our known rate
    ...
    def verify_and_extract_article(self, story_id, query):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Calculate relevance
                relevance_score = self.calculate_relevance(title, content, query.split())
    ...
    def comprehensive_intelligent_search(self, query, days_back=60, max_results=15):
        ...
        print(f"Query: '{query}' | Looking back: {days_back} days | Max results: {max_results}")
        print("=" * 70)
        ...
        unique_results.sort(key=lambda x: (x['relevance_score'], -x.get('estimated_age_days', 0)), reverse=True)
    ...
    def print_intelligent_results(self, results, duration, days_back):
        ...
        if results:
            ...
            for i, article in enumerate(results, 1):
                ...
                print(f"\n   {i}. {article['title'][:55]}...")
                print(f"      📊 Relevance: {article['relevance_score']} | Age: ~{age} days")
                print(f"      🔍 Method: {method}")
                print(f"      🆔 Story ID: {article['story_id']}")
                print(f"      📅 Published: {article.get('publish_date', 'Unknown')}")
...
def main():
    ...
    try:
        ...
        results = finder.comprehensive_intelligent_search(query, days_back=60, max_results=12)
