...
def deep_rss_pagination_search(target_ids=None, max_pages=20):
    ...
    for feed_url in rss_feeds:
                                        ...
                                        if link_elem:
                                            ...
                                            if story_match:
                                                ...
                                                if story_id in target_ids:
                                                    ...
                                                    feed_targets.append(target_info)
                                                    found_targets[story_id] = target_info
                                                    
                                                    print(f"   🎯 FOUND TARGET on page {page}: {story_id} - {title[:50]}...")
                                    
                                    if page_story_ids:
                                        id_range = f"{min(page_story_ids)}-{max(page_story_ids)}"
                                        print(f"   📄 Page {page}: {len(items)} items (IDs: {id_range})")
                                    else:
                                        print(f"   📄 Page {page}: {len(items)} items (no story IDs found)")
                                    
                                    break  # Found working pagination format
                                else:
                                    # Empty page - might be end of content
                                    print(f"   📄 Page {page}: Empty page, end of content")
                                    page_found = False
                                    break
        ...
        if feed_targets:
            print(f"   ✅ Feed summary: {feed_articles} articles scanned, {len(feed_targets)} targets found")
        else:
            print(f"   📊 Feed summary: {feed_articles} articles scanned, no targets found")
...
def comprehensive_rss_article_discovery(query="shellharbour council", max_pages=15):
    ...
    for feed_url in rss_feeds:
                ...
                if response.status_code == 200 and response.text.strip().startswith('<?xml'):
                    ...
                    for item in items:
                        ...
                        if title_elem and link_elem:
                            ...
                            # Check relevance
                            search_text = (title + " " + description + " " + link).lower()
                            relevance_score = 0
                    ...
                    if page_relevant > 0:
                        print(f"   📄 Page {page}: {len(items)} items, {page_relevant} relevant")
                    else:
        ...
        print(f"   📊 Feed summary: {feed_articles} total, {feed_relevant} relevant articles")
    
    # Sort by relevance
    all_relevant_articles.sort(key=lambda x: x['relevance_score'], reverse=True)
...
def main():
    ...
    relevant_articles, total_scanned = comprehensive_rss_article_discovery("shellharbour council", max_pages=20)
    ...
    if relevant_articles:
        ...
        for i, article in enumerate(relevant_articles[:10], 1):
            print(f"   {i}. Score {article['relevance_score']}: {article['title'][:60]}...")
            if article['story_id']:
                print(f"      ID: {article['story_id']} | Page: {article['page']}")
            print(f"      URL: {article['url']}")
            print()
    
    print("✅ DEEP RSS PAGINATION ANALYSIS COMPLETE!")
    print(f"💡 RSS pagination provides access to {total_scanned} total articles!")
