...
def discover_advanced_rss_patterns():
    ...
    categories = ['council', 'local', 'shellharbour', 'government', 'mayor', 'politics']
...
def analyze_sitemap_for_rss():
    ...
    try:
        # Get main sitemap
        response = requests.get('https://www.illawarramercury.com.au/sitemap.xml', headers=headers)
        if response.status_code == 200:
            content = response.text
            
            # Look for RSS references
            rss_urls = re.findall(r'https://[^<\s"]+(?:rss|feed)(?:[^<\s"]*\.xml)?[^<\s"]*', content, re.IGNORECASE)
            if rss_urls:
                print(f"   📡 Found {len(rss_urls)} RSS references in main sitemap:")
            ...
            for sitemap_url in sitemap_urls[:10]:  # Check first 10
                try:
                    ...
                    if sub_response.status_code == 200:
                        ...
                        sub_rss = re.findall(r'https://[^<\s"]+(?:rss|feed)[^<\s"]*', sub_content, re.IGNORECASE)
                        if sub_rss:
                            print(f"      📡 Found RSS in {sitemap_url.split('/')[-1]}: {len(sub_rss)} feeds")
...
def test_pagination_rss():
    ...
    for base_feed in base_feeds:
        ...
        for page_format in pagination_formats:
            ...
            for page in range(2, 6):  # Test pages 2-5
                try:
                    ...
                    if response.status_code == 200 and response.text.strip().startswith('<?xml'):
                        ...
                        if items:
                            found_pages += 1
                            print(f"   ✅ Page {page} found: {len(items)} items - {test_url}")
                                ...
                                if link_elem:
                                    ...
                                    if story_match:
                                        ...
                                        if story_id in ['9050660', '9046630', '9045329', '636609', '9044604']:
                                            print(f"      🎯 FOUND TARGET: {story_id} in {test_url}")
                        break  # Found working format, continue with this
                    else:
                        print(f"   ❌ Page {page}: {response.status_code} - {test_url}")
            ...
            if found_pages > 0:
                print(f"   📊 Found {found_pages} additional pages for this feed")
