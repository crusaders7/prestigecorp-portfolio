...
class GoogleAPIKeyDiscovery:
            ...
            for query in search_queries:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        found_keys = self._extract_api_keys_from_text(response.text)
        ...
        for pattern in self.api_key_patterns:
            ...
            for match in matches:
                ...
                if clean_key:
                    ...
                    if key not in keys and len(key) == 39:  # Google API keys are 39 chars
                        keys.append(key)
    ...
    def _test_api_keys(self, key_list: List[Dict]) -> List[Dict]:
        ...
        for i, key_info in enumerate(key_list, 1):
            ...
            try:
                ...
                if result['is_valid']:
                    print(f"   ✅ VALID KEY FOUND! Source: {key_info.get('source', 'unknown')}")
                    key_info['test_result'] = result
                    valid_keys.append(key_info)
                else:
                    print(f"   ❌ Invalid key. Error: {result.get('error', 'Unknown')}")
    ...
    def get_discovery_summary(self) -> Dict:
        ...
        try:
            ...
            cursor.execute('SELECT COUNT(*) FROM discovered_keys WHERE is_valid = 1')
            valid_keys = cursor.fetchone()[0]
            
            cursor.execute('SELECT source, COUNT(*) FROM discovered_keys GROUP BY source')
...
def main():
    ...
    print(f"\n📊 Discovery Summary:")
    print(f"   Total keys discovered: {summary.get('total_keys_discovered', 0)}")
