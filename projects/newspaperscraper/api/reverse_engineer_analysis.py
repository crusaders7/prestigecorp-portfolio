...
def analyze_url_structure(headers):
    ...
    try:
        ...
        response = requests.get('https://www.illawarramercury.com.au', headers=headers, timeout=10)
        if response.status_code == 200:
            ...
            for url in article_urls[:50]:  # Analyze first 50
                ...
                pattern_key = '/'.join(['*' if part.isdigit() else part for part in path_parts if part])
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if story_ids:
                ...
                patterns['estimated_daily_rate'] = id_range / 30  # Assume 30 days of content
                ...
                print(f"   📈 Current ID range: {min(ids_numeric)} - {max(ids_numeric)}")
                print(f"   ⚡ Estimated daily rate: ~{patterns['estimated_daily_rate']:.1f} articles/day")
                print(f"   📅 Estimated 1 month ago ID: {patterns['older_article_estimates']['1_month_ago']}")
                    ...
                    if 'xml' in url.lower() and 'xml' in response.headers.get('content-type', ''):
                        ...
                        for url_elem in urls_found[:20]:  # Check first 20
                            ...
                            if loc:
                                ...
                                date_match = re.search(r'(\d{4}/\d{2}/\d{2})', url_text)
                                if date_match:
                                    date_patterns.append(date_match.group(1))
                        
                        date_systems[url]['date_patterns'] = list(set(date_patterns))
                        
                        print(f"   ✅ Found sitemap: {len(urls_found)} URLs")
                        if date_patterns:
                            print(f"   📅 Date patterns found: {date_patterns[:3]}")
                else:
                    date_systems[url] = {'status': 'not_found', 'code': response.status_code}
            except Exception:
        ...
        if response.status_code == 200:
            ...
            for script in inline_scripts:
                if script.string:
                    ...
                    api_matches = re.findall(r'["\']([^"\']*(?:api|search|story)[^"\']*)["\']', script.string, re.IGNORECASE)
                    js_apis['apis'].extend([{'endpoint': match, 'source_js': 'inline', 'pattern': 'inline'} for match in api_matches])
...
def analyze_database_hints(headers):
    ...
    try:
        ...
        response = requests.get('https://www.illawarramercury.com.au/robots.txt', headers=headers, timeout=10)
        if response.status_code == 200:
            ...
            print(f"   🤖 robots.txt analysis: {len(db_paths)} database-related paths")
