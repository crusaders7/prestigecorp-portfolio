...
def test_enhanced_categories():
    ...
    try:
        ...
        # Quick quality check
        shellharbour_count = sum(1 for url in results if 'shellharbour' in url.lower())
        council_count = sum(1 for url in results if 'council' in url.lower())
        
        print(f"\n🎯 RELEVANCE ANALYSIS:")
        print(f"📍 URLs with 'shellharbour': {shellharbour_count}/{len(results)} ({shellharbour_count/len(results)*100:.1f}%)")
        print(f"🏛️  URLs with 'council': {council_count}/{len(results)} ({council_count/len(results)*100:.1f}%)")
...
def test_category_coverage():
    ...
    try:
        ...
        print(f"📈 Average per category: {total_articles/working_categories:.1f}")
...
def main():
    ...
    if search_results:
        ...
        print(f"   🎯 Relevance: {search_results['shellharbour_results']} Shellharbour matches")
    
    if coverage_results:
        print(f"📊 Category Coverage:")
        print(f"   ✅ Working: {coverage_results['working_categories']}/14 categories")
        print(f"   📰 Articles: {coverage_results['total_articles']} total available")
