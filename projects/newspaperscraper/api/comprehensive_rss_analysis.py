...
def discover_all_rss_feeds():
    ...
    print("\n📋 Checking robots.txt...")
    try:
        response = requests.get('https://www.illawarramercury.com.au/robots.txt', headers=headers)
        if response.status_code == 200:
            content = response.text
            print(f"   ✅ robots.txt accessible ({len(content)} chars)")
            
            # Look for RSS/feed references
            feed_urls = re.findall(r'https://[^\s]+(?:rss|feed|xml)[^\s]*', content, re.IGNORECASE)
            if feed_urls:
                print(f"   📡 Found {len(feed_urls)} feed references in robots.txt:")
    ...
    try:
        response = requests.get('https://www.illawarramercury.com.au/', headers=headers)
        if response.status_code == 200:
            ...
            feed_links = [link['href'] for link in all_links if 'rss' in link['href'].lower() or 'feed' in link['href'].lower()]
            if feed_links:
...
def analyze_rss_feed_content(feed_url):
    ...
    try:
        ...
        if analysis['story_ids']:
            ...
            print(f"   📈 Story ID range: {analysis['id_range']['min']} - {analysis['id_range']['max']} (span: {analysis['id_range']['span']})")
        
        # Show categories
        if analysis['categories']:
            print(f"   📂 Categories: {', '.join(list(analysis['categories'])[:5])}{'...' if len(analysis['categories']) > 5 else ''}")
        
        # Show Shellharbour matches
        if analysis['shellharbour_articles']:
            print(f"   🎯 Shellharbour articles: {len(analysis['shellharbour_articles'])}")
        ...
        if analysis['council_articles']:
            print(f"   🏛️ Council articles: {len(analysis['council_articles'])}")
    ...
    for feed_url in working_feeds:
        ...
        if analysis and analysis['articles']:
            ...
            if relevant_articles:
                print(f"   🎯 Found {len(relevant_articles)} relevant articles in this feed")
    ...
    for feed_url in working_feeds:
        ...
        if analysis:
            ...
            if analysis.get('id_range'):
                print(f"   📈 ID range: {analysis['id_range']['min']} - {analysis['id_range']['max']}")
            print(f"   🎯 Shellharbour content: {len(analysis['shellharbour_articles'])} articles")
            print(f"   🏛️ Council content: {len(analysis['council_articles'])} articles")
...
def test_specific_story_ids_in_rss():
    ...
    print(f"   ❌ Missing from RSS: {set(target_ids) - set(found_targets.keys())}")
...
if __name__ == "__main__":
    ...
    print(f"💡 Discovered {len(feeds)} working RSS feeds with enhanced search capabilities")
