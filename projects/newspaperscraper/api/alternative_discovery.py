    ...
    for rss_url in rss_urls:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                story_urls = re.findall(r'https://www\.illawarramercury\.com\.au/story/\d+/[^<\s"]+', content)
                ...
                for params in search_params:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if story_links:
                                print(f'   📊 Found {len(story_links)} results for "{term}"')
    ...
    for sitemap_url in sitemap_urls:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                additional_sitemaps = re.findall(r'https://[^<\s"]+sitemap[^<\s"]*\.xml', content)
                if additional_sitemaps:
                    print(f'   📂 Found {len(additional_sitemaps)} additional sitemaps')
                    for add_sitemap in additional_sitemaps[:5]:  # Test first 5
                        print(f'   📋 Testing additional: {add_sitemap}')
                        try:
                            add_response = requests.get(add_sitemap, headers=headers, timeout=10)
                            if add_response.status_code == 200:
                                add_content = add_response.text
                                add_story_urls = re.findall(r'https://www\.illawarramercury\.com\.au/story/\d+/[^<\s"]+', add_content)
                                if add_story_urls:
                                    print(f'      📰 Found {len(add_story_urls)} additional stories')
