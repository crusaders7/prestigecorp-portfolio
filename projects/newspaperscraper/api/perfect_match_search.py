...
class PerfectMatchSearchSystem:
                    ...
                    for start_id, end_id in sample_ranges:
                        ...
                        for story_id in range(start_id, end_id, 5):
                            try:
                                ...
                                response = requests.get(story_url, headers=self.headers, timeout=5)
                                ...
                                if response.status_code == 200:
                                    ...
                                    if relevance_score >= 1:
                                        print(f"   🎯 {story_id}: Score {relevance_score} - {title[:60]}...")
    ...
    def comprehensive_category_scan(self, query_terms, max_articles_per_category=100):
        ...
        for i, category in enumerate(self.comprehensive_categories):
            ...
            try:
                response = requests.get(category, headers=self.headers, timeout=10)
                if response.status_code == 200:
                    soup = BeautifulSoup(response.content, 'html.parser')
                    ...
                    for script in soup.find_all('script'):
                        if script.string:
                            story_urls = re.findall(r'https://www\.illawarramercury\.com\.au/story/\d+/[^"\']+', script.string)
                            article_links.update(story_urls)
                    
                    category_articles = list(article_links)[:max_articles_per_category]
                    ...
                    if category_articles:
                        ...
                        if relevant_count > 0:
                            print(f"   🎯 {relevant_count} potentially relevant articles")
                    else:
    ...
    def perfect_match_search(self, query="shellharbour council"):
        ...
        print("\n📊 PHASE 2: COMPREHENSIVE CATEGORY SCANNING")
        all_articles, relevant_articles = self.comprehensive_category_scan(query_terms)
    ...
    def print_results(self, results):
        ...
        if results['perfect_matches']:
            ...
            for i, match in enumerate(results['perfect_matches'][:10], 1):
                print(f"   {i}. Score {match['score']}: {match['title'][:60]}...")
        ...
        print(f"\n✅ SEARCH COMPLETE!")
        print(f"💡 Found {len(results['perfect_matches'])} perfect matches for enhanced targeting!")
