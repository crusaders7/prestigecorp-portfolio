    ...
    try:
        ...
        for header in problematic:
            if header in headers:
                ...
                if value == '*':
                    print(f"⚠️  {header}: {value} (Too permissive for gov networks)")
...
def test_network_connectivity():
    ...
    try:
        # Test HTTP redirect
        http_response = requests.get("http://news.prestigecorp.au", timeout=10, allow_redirects=False)
        ...
        # Test HTTPS directly
        https_response = requests.get("https://news.prestigecorp.au", timeout=10)
...
def test_government_network_compatibility():
    ...
    try:
        ...
        if headers.get('Access-Control-Allow-Origin') == '*':
            issues.append("Permissive CORS policy may trigger security filters")
