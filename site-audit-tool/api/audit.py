    ...
    try:
        ...
        for rec in results['recommendations']:
            if any(word in rec.lower() for word in ['critical', 'missing', 'https', 'security']):
                ...
                priority_recommendations.append({'text': rec, 'priority': 'low'})
...
def analyze_seo(soup, url):
    ...
    if title and title.get_text().strip():
        ...
        if title_length > 0:
            ...
            if title_length <= 60:
                ...
                issues.append(f"Title tag is {title_length} characters (recommended: 50-60)")
                recommendations.append("Shorten your title tag to 50-60 characters for better search engine display")
        else:
            issues.append("Title tag is empty")
            recommendations.append("Add a descriptive title tag with your primary keywords")
    else:
        issues.append("Missing title tag")
        recommendations.append("Add a title tag to improve search engine visibility")
    
    # Favicon analysis
    favicon = soup.find('link', rel=lambda x: x and 'icon' in x.lower()) or soup.find('link', rel='shortcut icon')
    if favicon:
        score += 5
    else:
        recommendations.append("Add a favicon to improve brand recognition and user experience")
    ...
    if meta_desc and meta_desc.get('content', '').strip():
        ...
        if desc_length < 120:
            recommendations.append("Meta description could be longer for better search engine display (120-160 characters)")
        elif desc_length > 160:
            issues.append(f"Meta description is {desc_length} characters (recommended: 120-160)")
            recommendations.append("Shorten your meta description to 120-160 characters")
        else:
            score += 5  # Bonus for optimal length
    else:
        issues.append("Missing meta description")
        recommendations.append("Add a compelling meta description to improve click-through rates")
    
    # Keywords meta tag (discouraged but still check)
    meta_keywords = soup.find('meta', attrs={'name': 'keywords'})
    if meta_keywords:
        recommendations.append("Meta keywords tag is deprecated and ignored by search engines - consider removing")
    ...
    if robots_meta:
        ...
        if 'noindex' in robots_content:
            issues.append("Page is set to noindex - it won't appear in search results")
        score += 3
    else:
        recommendations.append("Consider adding robots meta tag for explicit indexing control")
    ...
    if h1_tags:
        ...
        if len(h1_tags) == 1:
            ...
            if len(h1_text) > 70:
                recommendations.append("H1 tag is quite long - consider shortening for better readability")
        elif len(h1_tags) > 1:
            issues.append(f"Multiple H1 tags found ({len(h1_tags)})")
            recommendations.append("Use only one H1 tag per page for better SEO structure")
    else:
    ...
    if json_ld_scripts:
        ...
        try:
            ...
            recommendations.append("JSON-LD schema markup found but may have syntax errors")
    else:
        recommendations.append("Add structured data (Schema.org) markup to help search engines understand your content")
    ...
    if len(headings) >= 2:
        ...
        recommendations.append("Use proper heading hierarchy (H1, H2, H3) to structure your content")
    ...
    og_score = 0
    if og_title: og_score += 5
    if og_desc: og_score += 5
    if og_image: og_score += 5
    ...
    if html_lang and html_lang.get('lang'):
        ...
        recommendations.append("Add lang attribute to html element for better accessibility and SEO")
    ...
    if charset:
        ...
        recommendations.append("Add meta charset declaration (preferably UTF-8)")
    ...
    if preload_links or prefetch_links:
        ...
        recommendations.append("Consider using resource hints (preload/prefetch) for critical resources")
...
def analyze_performance(response, soup):
    ...
    if hasattr(response, 'elapsed'):
        ...
        if response_time < 0.5:
            ...
            score += 15
            recommendations.append(f"Response time is {response_time:.2f}s - consider optimizing server response")
        elif response_time < 3:
            score += 10
            issues.append(f"Response time is {response_time:.2f}s (slow: should be <2s)")
            recommendations.append("Optimize server response time with caching and CDN")
        else:
            issues.append(f"Response time is {response_time:.2f}s (very slow: should be <1s)")
            recommendations.append("Critical: Server response time is very slow. Immediate optimization needed")
    ...
    if content_size < 50000:  # Less than 50KB
        ...
        score += 5
        issues.append(f"Page size is {content_size_mb:.1f}MB (recommended: <0.5MB)")
        recommendations.append("Optimize images and minify CSS/JS to reduce page size")
    else:
        issues.append(f"Page size is {content_size_mb:.1f}MB (large: should be <1MB)")
        recommendations.append("Page is very large. Compress images, minify code, and use lazy loading")

    # Images optimization analysis
    images = soup.find_all('img')
    if images:
        images_with_alt = len([img for img in images if img.get('alt')])
        images_without_srcset = len([img for img in images if not img.get('srcset')])
        images_without_loading = len([img for img in images if not img.get('loading')])
        ...
        if images_without_srcset > len(images) * 0.7:  # More than 70% missing srcset
            recommendations.append(f"Add responsive images (srcset) for better performance on different devices")
        
        if images_without_loading > len(images) * 0.5:  # More than 50% missing lazy loading
            recommendations.append(f"Add lazy loading to improve initial page load time")
            
        if images_without_width_height > len(images) * 0.5:
            recommendations.append("Add width and height attributes to images to prevent layout shift")
            
        # Check for oversized images
        large_images = [img for img in images if img.get('src') and any(ext in img.get('src', '').lower() for ext in ['.jpg', '.jpeg', '.png', '.gif'])]
        if len(large_images) > 5:
            recommendations.append("Consider using modern image formats like WebP or AVIF for better compression")
    else:
    ...
    if len(css_files) <= 2:
        ...
        css_score += 5
        issues.append(f"Multiple CSS files ({len(css_files)}) may slow page loading")
        recommendations.append("Combine and minify CSS files to improve loading speed")
    else:
    ...
    if len(inline_css) > 5:
        recommendations.append("Minimize inline CSS and move to external files for better caching")
    
    # Check for CSS minification opportunities
    for css_link in css_files:
        href = css_link.get('href', '')
        if '.min.' not in href and 'min.css' not in href:
            recommendations.append("Consider using minified CSS files for better performance")
            break
    ...
    if len(js_files) <= 3:
        ...
        js_score += 5
        issues.append(f"Multiple JavaScript files ({len(js_files)}) may impact performance")
    ...
    blocking_js = [js for js in js_files if not js.get('async') and not js.get('defer')]
    if len(blocking_js) > 2:
        recommendations.append("Add async or defer attributes to non-critical JavaScript")
    
    # Check for JS minification
    for js_script in js_files:
        src = js_script.get('src', '')
        if '.min.' not in src and 'min.js' not in src:
            recommendations.append("Consider using minified JavaScript files for better performance")
            break
    ...
    font_preloads = soup.find_all('link', attrs={'rel': 'preload', 'as': 'font'})
    
    if font_links and not font_preloads:
        recommendations.append("Consider preloading critical fonts to reduce layout shift")
    ...
    if external_resources:
        ...
        recommendations.append("Consider using a CDN for static assets to improve global loading times")
...
def analyze_accessibility(soup):
    ...
    images = soup.find_all('img')
    if images:
        images_with_alt = len([img for img in images if img.get('alt') is not None])  # Include empty alt=""
        images_with_meaningful_alt = len([img for img in images if img.get('alt', '').strip()])
        images_decorative = len([img for img in images if img.get('alt') == ''])  # Properly marked decorative
        ...
        if images_with_alt == len(images):
            ...
            recommendations.append("Add alt attributes to all images - use descriptive text or empty alt='' for decorative images")
        
        # Check for overly long alt text
        long_alt_images = [img for img in images if len(img.get('alt', '')) > 125]
        if long_alt_images:
            recommendations.append(f"{len(long_alt_images)} images have very long alt text - consider shortening to under 125 characters")
    else:
    ...
    if headings:
        ...
        if h1_count == 1:
            ...
            recommendations.append("Add an H1 heading to establish the main topic of the page")
        else:
        ...
        for i in range(len(unique_levels) - 1):
            if unique_levels[i + 1] - unique_levels[i] > 1:
                issues.append(f"Heading levels skip from H{unique_levels[i]} to H{unique_levels[i + 1]}")
                recommendations.append("Don't skip heading levels (e.g., H2 should follow H1, not H3)")
                break
        ...
        if empty_headings:
            ...
            recommendations.append("Remove empty headings or add meaningful content")
    else:
        issues.append("No headings found")
        recommendations.append("Use proper heading structure (H1-H6) to organize content for screen readers")
            ...
            for inp in inputs:
                ...
                if inp.get('required') and not (aria_label and 'required' in aria_label.lower()):
                    if not soup.find('label', attrs={'for': inp_id}, string=lambda text: text and '*' in text):
                        recommendations.append("Mark required fields clearly for screen readers")
            
            visible_inputs = len([inp for inp in inputs if inp.get('type', '').lower() != 'hidden'])
            if visible_inputs > 0:
                ...
                if label_ratio < 1:
                    ...
                    issues.append(f"{unlabeled} form inputs missing proper labels")
                    recommendations.append("Add labels, aria-label, or aria-labelledby to all form inputs")
        
        # Check for fieldsets in complex forms
        fieldsets = soup.find_all('fieldset')
        if len(inputs) > 5 and not fieldsets:
            recommendations.append("Use fieldsets with legends to group related form elements")
        elif fieldsets:
            form_score += 5
            # Check fieldsets have legends
            fieldsets_without_legend = [fs for fs in fieldsets if not fs.find('legend')]
            if fieldsets_without_legend:
                issues.append(f"{len(fieldsets_without_legend)} fieldsets missing legends")
                recommendations.append("Add legends to all fieldsets for better form structure")
        
        # Check for form validation and error handling
        error_elements = soup.find_all(class_=lambda x: x and ('error' in x.lower() or 'invalid' in x.lower()))
        if not error_elements and len(inputs) > 2:
            recommendations.append("Implement accessible error messaging for form validation")
    ...
    if html_tag and html_tag.get('lang'):
        ...
        if len(lang_value) < 2:
            ...
            recommendations.append("Use proper language codes (e.g., 'en', 'en-US') for the lang attribute")
    else:
        issues.append("Missing lang attribute on html element")
        recommendations.append("Add lang attribute to html element for screen readers")
    
    # Page title
    title = soup.find('title')
    if not title or not title.get_text().strip():
        issues.append("Missing or empty page title")
        recommendations.append("Add a descriptive page title for screen readers and browser tabs")
    else:
        score += 5
    
    # Skip navigation links
    skip_links = soup.find_all('a', href=lambda x: x and x.startswith('#'))
    has_skip_nav = any('skip' in link.get_text().lower() or 'jump' in link.get_text().lower() for link in skip_links)
    if has_skip_nav:
        score += 5
    else:
    ...
    if links:
        ...
        for link in links:
            ...
            if link.get('target') == '_blank' and not aria_label and 'new window' not in link_text.lower():
                recommendations.append("Links opening in new windows should inform users (add 'opens in new window' to link text or aria-label)")
        
        if empty_links:
            issues.append(f"{len(empty_links)} links without descriptive text")
            recommendations.append("Ensure all links have descriptive text or aria-label")
        
        if link_issues > 0:
            recommendations.append(f"{link_issues} links have non-descriptive text like 'click here' - use more meaningful link text")
    ...
    if buttons:
        ...
        if empty_buttons:
            issues.append(f"{len(empty_buttons)} buttons without descriptive text")
    ...
    positive_tabindex = [el for el in elements_with_tabindex if el.get('tabindex', '0').isdigit() and int(el.get('tabindex', '0')) > 0]
    
    if positive_tabindex:
        issues.append(f"{len(positive_tabindex)} elements use positive tabindex values")
        recommendations.append("Avoid positive tabindex values - use 0 or -1, or rely on natural tab order")
    
    # ARIA landmarks and roles
    landmarks = soup.find_all(attrs={'role': lambda x: x and x.lower() in ['main', 'navigation', 'banner', 'contentinfo', 'complementary']})
    semantic_elements = soup.find_all(['main', 'nav', 'header', 'footer', 'aside', 'section', 'article'])
    
    if landmarks or semantic_elements:
        score += 8
    else:
        recommendations.append("Use ARIA landmarks or semantic HTML5 elements to structure page content")
    
    # Color and contrast considerations
    recommendations.append("Ensure sufficient color contrast (4.5:1 for normal text, 3:1 for large text, 7:1 for enhanced)")
    ...
    if videos:
        videos_with_captions = [v for v in videos if v.find('track', kind='captions') or v.find('track', kind='subtitles')]
        if len(videos_with_captions) < len(videos):
            issues.append(f"{len(videos) - len(videos_with_captions)} videos missing captions")
            recommendations.append("Add captions or subtitles to all video content")
        else:
            score += 5
...
def analyze_security(response, url):
    ...
    if url.startswith('https://'):
        ...
        recommendations.append("Enable HTTPS encryption for secure data transmission - this is critical for user trust and SEO")
    ...
    if hsts_header:
        ...
        if 'max-age' in hsts_header:
            try:
                ...
                if max_age >= 31536000:  # 1 year
                    ...
                    recommendations.append("HSTS max-age is too short - consider at least 1 year")
            except:
                pass
        ...
        recommendations.append("Add Strict-Transport-Security header to enforce HTTPS and prevent downgrade attacks")
    ...
    if headers.get('X-Content-Type-Options', '').lower() == 'nosniff':
        ...
        recommendations.append("Add X-Content-Type-Options: nosniff header to prevent MIME sniffing attacks")
    ...
    if x_frame_options:
        ...
        if x_frame_options.upper() not in ['DENY', 'SAMEORIGIN']:
            recommendations.append("X-Frame-Options should be set to DENY or SAMEORIGIN")
    ...
    if csp_header:
        ...
        if 'unsafe-eval' in csp_header:
            ...
            recommendations.append("Remove 'unsafe-eval' from Content Security Policy")
        if '*' in csp_header and 'data:' not in csp_header:
            recommendations.append("CSP uses wildcards - consider more restrictive policies")
        
        # Check for important directives
        important_directives = ['default-src', 'script-src', 'style-src', 'img-src']
        missing_directives = [directive for directive in important_directives if directive not in csp_header]
        if missing_directives:
            recommendations.append(f"Consider adding CSP directives: {', '.join(missing_directives)}")
    
    if not has_frame_protection:
        issues.append("Missing clickjacking protection")
        recommendations.append("Add X-Frame-Options or Content-Security-Policy frame-ancestors directive to prevent clickjacking")
    ...
    if xss_protection:
        if xss_protection == '1; mode=block':
            ...
            recommendations.append("X-XSS-Protection should be '1; mode=block' or removed in favor of CSP")
    else:
        if not csp_header:  # Only recommend if no CSP
            recommendations.append("Consider adding X-XSS-Protection header (though CSP is preferred)")
    ...
    if referrer_policy:
        ...
        recommendations.append("Add Referrer-Policy header to control referrer information leakage")
    ...
    if permissions_policy:
        ...
        recommendations.append("Consider adding Permissions-Policy header to control browser features")
    ...
    if corp or coop or coep:
        header_scores += 5
        recommendations.append("Good use of Cross-Origin headers for enhanced security")
    else:
        recommendations.append("Consider implementing Cross-Origin security headers (CORP, COOP, COEP)")
    ...
    if server_header:
        if any(server in server_header.lower() for server in ['apache', 'nginx', 'iis', 'express']):
            recommendations.append("Consider hiding or customizing Server header to reduce information disclosure")
    else:
    ...
    revealing_headers = ['X-AspNet-Version', 'X-AspNetMvc-Version', 'X-Generator']
    for header in revealing_headers:
        if header in headers:
            recommendations.append(f"Remove {header} header to reduce information disclosure")
    ...
    if url.startswith('https://'):
        ...
        recommendations.append("Ensure all resources (images, scripts, stylesheets) are loaded over HTTPS to avoid mixed content warnings")
    ...
    if hasattr(response, 'url') and response.url != url:
        if not response.url.startswith('https://') and url.startswith('https://'):
            issues.append("HTTPS request redirected to HTTP - potential security issue")
