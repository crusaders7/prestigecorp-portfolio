...
def test_pagination_improvements():
    ...
    test_cases = [
        ...
        {'max_results': 15, 'expected_min': 15,
            'description': '15 results (requires pagination)'},
        {'max_results': 20, 'expected_min': 15,
            'description': '20 results (requires 2 API calls)'},
        {'max_results': 25, 'expected_min': 20,
            'description': '25 results (requires 3 API calls)'}
    ]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if found >= expected_min:
                    print(
                        f'   ✅ SUCCESS: Got {found} results (expected min: {expected_min})')
                elif found >= 10:
                    print(
                        f'   ⚠️  PARTIAL: Got {found} results (may be limited by available content)')
                else:
                    print(
                        f'   ❌ FAILED: Only got {found} results (expected min: {expected_min})')
                ...
                if 'cost_info' in data and 'estimated_cost' in data['cost_info']:
                    print(
                        f'   💰 Estimated cost: ${data["cost_info"]["estimated_cost"]:.3f}')
